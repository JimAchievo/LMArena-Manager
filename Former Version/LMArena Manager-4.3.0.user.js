...
(function() {
    ...
    const DEFAULT_COMPANY_ORDER = [
        ...
        'Perplexity', 'Diffbot',
        // Image
        'Bytedance', 'Flux', 'MicrosoftAI', 'ShengShu', 'Recraft',
        'Luma', 'Ideogram', 'Reve', 'LeonardoAI',
        // Code
        'Kwai'
    ];
    ...
    const ARENA_MODE_RULES = [
        ...
        {
            mode: 'code',
            patterns: [/codex/i, /^devstral/i, /^kat/i, /grok-code/i],
            exclude: [/^qwen.*coder/i]  // qwen-coder 保留在 LLM
        },
        {
            mode: 'video',
            patterns: [/video/i]
    ...
    class DataManager {
        ...
        analyze(name, svgCompany) {
            ...
            if (svgCompany && svgCompany !== '') {
                if (svgCompany.toLowerCase() === 'zhipu') {
                    company = 'Zai';
                } else if (svgCompany.toLowerCase().includes('microsoft')) {
                    company = 'MicrosoftAI';
                } else {
                    company = svgCompany;
                }
                for (const rule of COMPANY_RULES) {
                    if (rule.company.toLowerCase() === company.toLowerCase()) {
                        icon = rule.icon;
            ...
            for (const rule of ARENA_MODE_RULES) {
                if (rule.exclude && rule.exclude.some(p => p.test(name))) {
                    continue;
                }
                if (rule.patterns.some(p => p.test(name))) {
                    arenaMode = rule.mode;
    ...
    class Scanner {
        constructor(dm) {
            ...
            this.scanSession = {
                active: false,
                startedAt: null,
                scannedModels: new Set(),
                scannedModes: new Set()
            };
        }
        ...
        scan() {
            ...
            options.forEach(el => {
                ...
                if (this.scanSession.active) {
                    ...
                    const existingModel = this.dm.getModel(info.name);
                    if (existingModel) {
                        this.scanSession.scannedModes.add(existingModel.arenaMode);
                    }
                }
                ...
                if (!this.dm.getModel(info.name)) {
                    ...
                    if (this.scanSession.active) {
                        this.scanSession.scannedModes.add(data.arenaMode);
                    }
                }
            });
        ...
        endScanSession() {
            ...
            const result = {
                ...
                modes: [...this.scanSession.scannedModes]
            };
    ...
    class UI {
        ...
        injectStyles() {
            GM_addStyle(`
                ...
                .lmm-panel {
                    ...
                    width: 96vw; max-width: 1000px; height: 88vh; max-height: 720px;
                ...
                .lmm-topbar {
                    ...
                    flex-wrap: wrap; /* 允许换行，修复滚动条 */
                }
                .lmm-topbar-item {
                    padding: 4px 8px; border-radius: 12px; border: 1px solid var(--lmm-border);
                    background: var(--lmm-bg); font-size: 12px; cursor: pointer;
                ...
                .lmm-topbar-item .cnt {
                    font-size: 10px; background: rgba(0,0,0,0.1); padding: 1px 5px; border-radius: 8px;
                }
                ...
                .lmm-sidebar-item .icon {
                    display: inline-flex; width: 1.4em; justify-content: center; flex-shrink: 0;
                }
                ...
                .lmm-drag-handle { cursor: grab; color: var(--lmm-text2); font-size: 12px; }

                /* 折叠夹 */
                .lmm-sidebar-folder {
                    display: flex; align-items: center; gap: 5px;
                    padding: 5px 8px; border-radius: 5px; cursor: pointer;
                    font-size: 11px; color: var(--lmm-text2); transition: all 0.1s;
                }
                ...
                .lmm-card-btn {
                    font-size: 12px; background: var(--lmm-bg2);
                ...
                .lmm-modal {
                    ...
                    z-index: 100002; min-width: 320px; max-width: 90vw; max-height: 80vh; overflow-y: auto;
                ...
                .lmm-form-input, .lmm-form-select {
        ...
        getModeCounts() {
            ...
            const counts = { all: models.length, llm: 0, search: 0, image: 0, code: 0, video: 0, starred: 0 };
            models.forEach(m => {
                if (counts[m.arenaMode] !== undefined) counts[m.arenaMode]++;
            ...
            panel.innerHTML = `
                ...
                <div class="lmm-toolbar">
                    ...
                    <select class="lmm-select" id="lmm-sort">
                        <option value="starred-desc">⭐ 收藏优先</option>
        ...
        createScanResultModal() {
            ...
            modal.innerHTML = `
                <div class="lmm-modal-title">🔍 扫描结果</div>
                <div class="lmm-modal-body">
                    <p id="lmm-scan-summary"></p>
                    <div class="lmm-scan-list" id="lmm-scan-list"></div>
                </div>
                <div class="lmm-modal-footer">
                    <button class="lmm-btn" id="lmm-scan-keep">保留全部</button>
                    <button class="lmm-btn lmm-btn-danger" id="lmm-scan-delete">删除选中</button>
                </div>
            `;
            document.body.appendChild(modal);
        ...
        showScanResult(result) {
            ...
            this.scanModal.querySelector('#lmm-scan-summary').innerHTML = `
                本次扫描到 <b>${scannedCount}</b> 个模型，
                ${missing.length > 0
                    ? `以下 <b>${missing.length}</b> 个模型未被扫描到（可能已下架）：`
                    : `所有模型均已扫描到，无需清理 ✓`
                }
            `;
            ...
            if (missing.length > 0) {
                list.innerHTML = `
                    <div class="lmm-scan-item" style="font-weight:500;background:var(--lmm-bg3)">
                        <input type="checkbox" id="lmm-scan-all" checked>
                        <label for="lmm-scan-all">全选</label>
                    </div>
                    ${missing.map(name => `
                        <div class="lmm-scan-item">
                            <input type="checkbox" class="lmm-scan-check" value="${this.esc(name)}" checked>
        ...
        bindEvents() {
            ...
            this.$('#lmm-settings').onclick = () => {
                ...
                this.showConfirm(
                    '⚙️ 设置',
                    `自动应用当前${autoApply ? '已开启' : '已关闭'}，是否${autoApply ? '关闭' : '开启'}？`,
                    () => {
        ...
        updateTopbar() {
            ...
            const items = [
                ...
                { key: 'llm', icon: '💬', label: 'LLM', count: counts.llm },
            ...
            topbar.innerHTML = items.map(it => `
                <div class="lmm-topbar-item ${this.currentMode === it.key ? 'active' : ''}" data-mode="${it.key}">
                    ${it.icon} ${it.label}
                    ${it.count > 0 ? `<span class="cnt">${it.count}</span>` : ''}
                </div>
            `).join('') + `
                <div class="lmm-topbar-sep"></div>
                <div class="lmm-topbar-item ${this.currentMode === 'starred' ? 'active' : ''}" data-mode="starred">
                    ⭐ 收藏 ${counts.starred > 0 ? `<span class="cnt">${counts.starred}</span>` : ''}
                </div>
                <div class="lmm-topbar-item ${this.currentMode === 'visible' ? 'active' : ''}" data-mode="visible">
                    👁️ 已启用
                </div>
                <div class="lmm-topbar-item ${this.currentMode === 'hidden' ? 'active' : ''}" data-mode="hidden">
                    🙈 已隐藏
                </div>
                <div class="lmm-topbar-item ${this.currentMode === 'new' ? 'active' : ''}" data-mode="new">
                    ✨ 新发现
                </div>
            `;
        ...
        getModelsInCurrentMode() {
            ...
            switch (this.currentMode) {
                ...
                case 'visible': return models.filter(m => m.visible !== false);
                case 'hidden': return models.filter(m => m.visible === false);
                case 'new': return models.filter(m => m.isNew);
                default: return models.filter(m => m.arenaMode === this.currentMode);
            }
        }
        ...
        updateSidebar() {
            ...
            // 修正后的折叠逻辑：除了 search, image, code, video 之外的模式（如 all, llm）使用折叠夹
            ...
            let html = `
                <div class="lmm-sidebar-header">
                    ...
                    <button class="lmm-sidebar-btn ${this.isSortMode ? 'active' : ''}" id="lmm-sort-btn">
                        ${this.isSortMode ? '完成' : '排序'}
                    </button>
                </div>
            ...
            Object.entries(companies).forEach(([name, data]) => {
                ...
                if (TIER1_COMPANIES.includes(name)) {
                    tier1.push(item);
                } else if (useFolder && TIER2_COMPANIES.includes(name)) {
                    tier2.push(item);
                } else {
                    // 如果不使用折叠夹，或者公司不在Tier表里，视为"Other"或直接展示
                    if (name !== 'Other') {
                        other.push(item);
                    }
                }
            ...
            const sortByOrder = arr => arr.sort((a, b) => {
                const ai = companyOrder.indexOf(a.name);
                const bi = companyOrder.indexOf(b.name);
            ...
            const hasOther = companies['Other'];
            ...
            if (showCategories) {
                ...
                if (catList) {
                    catList.innerHTML = Object.entries(cats)
                        .filter(([_, cnt]) => cnt > 0)
                        .map(([cat, cnt]) => `
                            <div class="lmm-sidebar-item ${this.filter.category === cat ? 'active' : ''}" data-cat="${cat}">
                                <span class="icon">${catLabels[cat].icon}</span>
        ...
        renderCompanyList(tier1, tier2, tier2Total, hasOther, other) {
            ...
            // 允许拖拽：SortMode下，无论是否在文件夹内，都允许
            const renderItem = (c, inFolder = false) => `
                <div class="lmm-sidebar-item ${this.isSortMode ? 'sort-mode' : ''} ${this.filter.company === c.name ? 'active' : ''}"
                     data-company="${this.esc(c.name)}" ${this.isSortMode ? 'draggable="true"' : ''}>
                    ${this.isSortMode ? '<span class="lmm-drag-handle">⠿</span>' : ''}
                    <span class="icon">${c.icon}</span>
                    <span style="flex:1;overflow:hidden;text-overflow:ellipsis">${this.esc(c.name)}</span>
            ...
            if (tier2.length > 0) {
                html += `
                    <div class="lmm-sidebar-folder" id="lmm-tier2-folder">
                        <span class="icon">${this.isTier2Expanded ? '📂' : '📁'}</span>
                        <span>更多公司</span>
                        <span class="cnt">${tier2Total}</span>
                    </div>
                    <div class="lmm-sidebar-folder-content ${this.isTier2Expanded ? 'open' : ''}" id="lmm-tier2-content">
                        ${tier2.map(c => renderItem(c, true)).join('')}
                    </div>
                `;
            }
            ...
            // Other 放最后
            if (hasOther) {
                html += renderItem({ name: 'Other', icon: '❔', cnt: hasOther.cnt });
            }
        ...
        toggleSortMode() {
            ...
            if (!this.isSortMode) {
                // 保存排序：选择所有带有 data-company 的元素，包括文件夹内的
                ...
                // 进入排序模式时，自动展开文件夹以便于操作
                if(this.$('#lmm-tier2-folder')) {
                    this.isTier2Expanded = true;
                }
            }
        ...
        bindDragEvents(item) {
            ...
            item.ondrop = (e) => {
                ...
                const to = item.dataset.company;

                if (from && to && from !== to) {
                    const order = this.dm.getCompanyOrder();
                    const fromIdx = order.indexOf(from);

                    if (fromIdx !== -1) {
                        order.splice(fromIdx, 1);
                        const toIdx = order.indexOf(to); // 重新获取索引因为数组变了
                        order.splice(toIdx === -1 ? order.length : toIdx, 0, from);
                        this.dm.setCompanyOrder(order);
                        this.updateSidebar();
                    }
                }
            };
        ...
        refresh() {
            ...
            if (models.length === 0) {
                grid.innerHTML = `<div class="lmm-empty" style="grid-column:1/-1">
                    ...
                    <div>没有匹配的模型<br><br>请打开模型下拉框以触发自动扫描</div>
                </div>`;
            } else {
                    ...
                    return `
                        <div class="lmm-card ${vis ? '' : 'hidden'} ${m.isNew ? 'new' : ''} ${m.starred ? 'starred' : ''}" data-name="${this.esc(m.name)}">
                            ...
                            <div class="lmm-card-actions">
                                <button class="lmm-card-btn lmm-star-btn ${m.starred ? 'starred' : ''}" title="收藏">
                                    ${m.starred ? '⭐' : '☆'}
                                </button>
                ...
                grid.querySelectorAll('.lmm-card').forEach(card => {
                    ...
                    card.onclick = (e) => {
                        ...
                        if (this.dm.data.settings.autoApply) {
                            this.scanner.applyFilters();
                        }
                    };
        ...
        batchSet(vis) {
            ...
            this.refresh();
            this.scanner.toast(`已${vis ? '显示' : '隐藏'} ${count} 个模型`, 'success');
