// ==UserScript==
// @name         LMArena Manager（该版本没有问题，未发布）
...
(function() {
    ...
    class UI {
        constructor(dm, scanner) {
            ...
            this.viewMode = 'grid';
            this.filter = { search: '', org: 'all', category: 'all', visibility: 'all', imageType: 'all', hasVision: 'all', group: 'all' };
        ...
        injectStyles() {
            GM_addStyle(`
                ...
                .lmm-search-hint { font-size: 10px; color: var(--lmm-text2); margin-top: 2px; padding-left: 4px; }
            ...
            panel.innerHTML = `
                <div class="lmm-header">
                    ...
                    <div class="lmm-header-btns">
                        ...
                        <button class="lmm-btn" id="lmm-groups-btn">📁 分组</button>
                ...
                <div class="lmm-toolbar">
                    <div class="lmm-search">
                        ...
                        <input type="text" placeholder="搜索... (空格=AND, /regex/)" id="lmm-search">
                    </div>
                    ...
                    <div class="lmm-view-toggle">
                        <button class="lmm-view-btn active" data-view="grid" title="网格视图">⊞</button>
                        <button class="lmm-view-btn" data-view="compact" title="紧凑视图">⊟</button>
                        <button class="lmm-view-btn" data-view="list" title="列表视图">☰</button>
                    </div>
                </div>
                ...
                <div class="lmm-footer"><div class="lmm-stats"><span class="lmm-stat">显示: <b id="lmm-v">0</b></span><span class="lmm-stat">隐藏: <b id="lmm-h">0</b></span><span class="lmm-stat">总计: <b id="lmm-t">0</b></span></div><span>Ctrl+Shift+M 打开 | / 聚焦搜索 | 双击编辑</span></div>
            `;
            document.body.appendChild(panel); this.panel = panel; this.bindEvents();
        }

        createEditModal() {
            const modalOverlay = document.createElement('div'); modalOverlay.className = 'lmm-modal-overlay'; modalOverlay.onclick = () => this.closeEditModal(); document.body.appendChild(modalOverlay); this.editModalOverlay = modalOverlay;
            const modal = document.createElement('div'); modal.className = 'lmm-modal';
            modal.innerHTML = `
                ...
                <div class="lmm-modal-body">
                    ...
                    <div class="lmm-form-group"><label class="lmm-form-label">所属分组</label><div class="lmm-checkbox-group" id="lmm-edit-groups"></div></div>
                </div>
            ...
            `;
            document.body.appendChild(modal); this.editModal = modal;
            modal.querySelectorAll('#lmm-edit-modes .lmm-checkbox-item, #lmm-edit-categories .lmm-checkbox-item').forEach(item => { item.onclick = () => item.classList.toggle('checked'); });
        ...
        createGroupModal() {
            const modalOverlay = document.createElement('div'); modalOverlay.className = 'lmm-modal-overlay'; modalOverlay.onclick = () => this.closeGroupModal(); document.body.appendChild(modalOverlay); this.groupModalOverlay = modalOverlay;
            const modal = document.createElement('div'); modal.className = 'lmm-modal'; modal.style.minWidth = '360px';
            modal.innerHTML = `
                <div class="lmm-modal-title">📁 分组管理</div>
                <div class="lmm-modal-body">
                    <div class="lmm-form-group">
                        <div style="display:flex;gap:6px;">
                            <input type="text" class="lmm-form-input" id="lmm-group-new-name" placeholder="新分组名称" style="flex:1">
                            <button class="lmm-btn lmm-btn-primary" id="lmm-group-create">创建</button>
                        </div>
                    </div>
                    <div class="lmm-group-list" id="lmm-group-list"></div>
                </div>
                <div class="lmm-modal-footer">
                    <button class="lmm-btn" id="lmm-group-close">关闭</button>
                </div>
            `;
            document.body.appendChild(modal); this.groupModal = modal;
        ...
        bindShortcuts() {
            document.addEventListener('keydown', e => {
                ...
                if (e.key === 'Escape') {
                    if (this.groupModal.classList.contains('open')) this.closeGroupModal();
        ...
        close() { this.isOpen = false; this.isSortMode = false; this.isModelSortMode = false; this.updateGridView(); this.panel.classList.remove('open'); this.overlay.classList.remove('open'); }
        ...
        updateTopbar() {
            ...
            html += `<div class="lmm-topbar-item ${this.currentMode === 'visible' ? 'active' : ''}" data-mode="visible">👁️ 已启用</div>`;
            html += `<div class="lmm-topbar-item ${this.currentMode === 'hidden' ? 'active' : ''}" data-mode="hidden">🙈 已隐藏</div>`;
            html += `<div class="lmm-topbar-item ${this.currentMode === 'starred' ? 'active' : ''}" data-mode="starred">⭐ 收藏 ${counts.starred > 0 ? `<span class="cnt">${counts.starred}</span>` : ''}</div>`;
            html += `<div class="lmm-topbar-item ${this.currentMode === 'new' ? 'active' : ''}" data-mode="new">✨ 新发现</div>`;
            // 分组
            if (groups.length > 0) {
                html += `<div class="lmm-topbar-sep"></div>`;
                groups.forEach(name => {
            ...
            topbar.querySelectorAll('.lmm-topbar-item').forEach(item => {
                item.onclick = () => {
                    ...
                    this.filter = { search: '', org: 'all', category: 'all', visibility: 'all', imageType: 'all', hasVision: 'all', group: 'all' };
                    this.$('#lmm-search').value = ''; this.$('#lmm-org').value = 'all'; this.$('#lmm-category').value = 'all';
                    this.isTier2Expanded = false; this.isModelSortMode = false;
                    this.updateGridView();
        ...
        updateSidebar() {
            ...
            const orgs = {};
            modeModels.forEach(m => { const c = m.company || 'Other'; if (!orgs[c]) orgs[c] = { cnt: 0, icon: m.icon || '❔' }; orgs[c].cnt++; });
            ...
            const sortByOrder = arr => arr.sort((a, b) => { const ai = orgOrder.indexOf(a.name); const bi = orgOrder.indexOf(b.name); return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi); });
            sortByOrder(tier1); sortByOrder(tier2); sortByOrder(other);
            const tier2Total = tier2.reduce((sum, c) => sum + c.cnt, 0); const hasOther = orgs['Other'];
            ...
            if (showCategories) {
                ...
                if (catList) {
                    ...
                    catList.querySelectorAll('.lmm-sidebar-item').forEach(item => { item.onclick = () => { if (this.isSortMode) return; catList.querySelectorAll('.lmm-sidebar-item').forEach(i => i.classList.remove('active')); item.classList.add('active'); this.filter.category = item.dataset.cat; this.collapseTier2(); this.refresh(); }; });
                }
            }

            if (showImageTypes) {
                const imageTypes = { universal: 0, t2i: 0, i2i: 0 };
                modeModels.forEach(m => { if (m.imageType && imageTypes[m.imageType] !== undefined) imageTypes[m.imageType]++; });
                ...
                if (imgTypeList) {
                    ...
                    imgTypeList.querySelectorAll('.lmm-sidebar-item').forEach(item => { item.onclick = () => { imgTypeList.querySelectorAll('.lmm-sidebar-item').forEach(i => i.classList.remove('active')); item.classList.add('active'); this.filter.imageType = item.dataset.imgtype; this.collapseTier2(); this.refresh(); }; });
                }
            }
        ...
        bindDragEvents(item) {
            item.ondragstart = (e) => { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', item.dataset.org); item.classList.add('dragging'); };
            item.ondragend = () => item.classList.remove('dragging');
            item.ondragover = (e) => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; };
            item.ondrop = (e) => {
                e.preventDefault(); const from = e.dataTransfer.getData('text/plain'); const to = item.dataset.org;
                if (from && to && from !== to) {
                    const sidebarMode = this.getSidebarMode();
                    const order = this.dm.getOrgOrder(sidebarMode); const fromIdx = order.indexOf(from);
                    if (fromIdx !== -1) { order.splice(fromIdx, 1); const toIdx = order.indexOf(to); order.splice(toIdx === -1 ? order.length : toIdx, 0, from); this.dm.setOrgOrder(sidebarMode, order); this.updateSidebar(); }
                }
            };
        }
        ...
        updateOrgFilter() {
            ...
            orgs.sort((a, b) => {
                ...
                const ai = orgOrder.indexOf(a); const bi = orgOrder.indexOf(b);
                return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi);
            });
            const sel = this.$('#lmm-org'); const val = sel.value;
            ...
            if (this.currentMode === 'visible') {
                ...
                if (customOrder.length > 0) {
                    models.sort((a, b) => {
                        ...
                        const cai = orgOrder.indexOf(a.company); const cbi = orgOrder.indexOf(b.company);
                ...
                models.sort((a, b) => {
                    ...
                    const ai = orgOrder.indexOf(a.company); const bi = orgOrder.indexOf(b.company);
        ...
        refresh() {
            ...
            if (models.length === 0) {
                ...
                grid.innerHTML = models.map(m => {
                    ...
                    const groupTags = modelGroups.slice(0, 2).map(g => `<span class="lmm-tag group">📁</span>`).join('');
            ...
            this.updateStats(); this.updateOrgFilter();
        }
...
openEditModal(name) {
    ...
    if (allGroups.length > 0) {
        groupsContainer.innerHTML = allGroups.map(g =>
                ...
                groupsContainer.innerHTML = '<span style="color:var(--lmm-text2);font-size:11px">暂无分组，点击顶栏"📁 分组"创建</span>';
            }
...
resetEdit() {
    ...
    this.closeEditModal(); this.refresh(); this.updateSidebar(); this.updateTopbar(); this.scanner.toast('已恢复默认', 'success');
}
...
createGroup() {
    ...
    if (!name) { this.scanner.toast('请输入分组名称', 'warning'); return; }
    if (this.dm.createGroup(name)) {
        input.value = '';
        this.renderGroupList();
        this.updateTopbar();
        this.scanner.toast(`分组"${name}"已创建`, 'success');
    } else {
        this.scanner.toast('分组名称已存在', 'warning');
    }
}
...
renderGroupList() {
    ...
    if (names.length === 0) {
        list.innerHTML = '<div style="color:var(--lmm-text2);text-align:center;padding:20px">暂无分组</div>';
        return;
    }
    list.innerHTML = names.map(name => `
                <div class="lmm-group-item" data-group="${this.esc(name)}">
                    <span class="name">📁 ${this.esc(name)}</span>
                    <span style="color:var(--lmm-text2);font-size:10px">${groups[name].length} 个模型</span>
                    <div class="actions">
                        <button class="lmm-btn lmm-rename-btn">重命名</button>
                        <button class="lmm-btn lmm-btn-danger lmm-delete-btn">删除</button>
                    </div>
                </div>
            `).join('');
            ...
            list.querySelectorAll('.lmm-group-item').forEach(item => {
                ...
                item.querySelector('.lmm-rename-btn').onclick = () => {
                    ...
                    if (newName && newName.trim() && newName !== name) {
                        if (this.dm.renameGroup(name, newName.trim())) {
                            this.renderGroupList();
                            this.updateTopbar();
                            this.scanner.toast('已重命名', 'success');
                        } else {
                            this.scanner.toast('名称已存在', 'warning');
                        }
                    }
                };
                item.querySelector('.lmm-delete-btn').onclick = () => {
                    if (confirm(`确定删除分组"${name}"吗？`)) {
                        ...
                        this.renderGroupList();
                        this.updateTopbar();
                        this.scanner.toast('已删除', 'success');
                    }
                };
            });
        }
